...
module.exports = {
  ...
  getAllStudents: async (req, res) => {
    try {
      ...
      res
        .status(500)
        .send({
          message: "Couldn't fetch the list of students",
          err: err.message,
        });
    }
  },
    ...
    try {
      const studentInfo = await Student.findById({ _id: req.params.studentId })
        .populate([
          {
            path: "wishlist",
            select: ["courseName", "thumbnail", "price", "rating"],
            populate: {
              path: "authorName",
              model: "tutor",
      ...
      res
        .status(200)
        .send({ message: "Fetched Student details: ", studentInfo });
    } catch (error) {
      console.log("Error while fetching the student info", error);
      res
        .status(500)
        .send({
          message: "Couldn't fetch the info of student",
          error: error.message,
        });
    }
  },
  ...
  addToWishlist: async (req, res) => {
    try {
      const courseToWishlist = await Course.findById({
      ...
      if (
        !student.wishlist.includes(courseToWishlist._id) &&
        !student.enrolledCourses.includes(courseToWishlist._id)
      ) {
        ...
        res
          .status(200)
          .send({
            message: "Added the course to wishlist",
            wishListed: courseToWishlist,
            student,
          });
      } else {
        res
          ...
          .send({
            message:
              "You've either Enrolled to this course or Course is already added to the wishlist",
          });
      }
    } catch (error) {
      ...
      res
        .status(500)
        .send({
          message: "Couldn't add the item to wishlist",
          error: error.message,
        });
    }
  },
  ...
  removeFromWishlist: async (req, res) => {
    try {
      const removeFromWishlist = await Course.findById({
        _id: req.params.courseId,
      });
      ...
      if (indexOfCourse > -1) {
        ...
        res
          ...
          .send({
            ...
            removedItem: removeFromWishlist,
          });
      }

      const indexOfStudent = removeFromWishlist.wishlistedBy.indexOf(
        req.user.id
      );
      if (indexOfStudent > -1) {
        ...
        res
          .status(200)
          .send({
            message: "Student has not wishlisted this course",
            removedItem: removeFromWishlist,
          });
      }
      ...
      res
        .status(500)
        .send({
          message: "Couldn't remove the item to wishlist",
          error: error.message,
        });
    }
  },
  ...
  addToCart: async (req, res) => {
    try {
      ...
      if (
        !student.cart.includes(courseToAdd._id) &&
        !student.enrolledCourses.includes(courseToAdd._id)
      ) {
        student.cart.push(courseToAdd._id);
        await student.save();
        res
          .status(200)
          .send({ message: "Added the course to cart", cart: student.cart });
      } else {
        res
          .status(200)
          .send({
            message:
              "You've either enrolled to this course or item is already added to cart",
          });
      }
    } catch (error) {
      ...
      res
        .status(500)
        .send({
          message: "Couldn't adding the item to cart",
          error: error.message,
        });
    }
  },
  ...
  removeFromCart: async (req, res) => {
    try {
      ...
      res
        .status(500)
        .send({
          message: "Couldn't removing the item from cart",
          error: error.message,
        });
    }
  },
  ...
  getLatestCourse: async (req, res) => {
    try {
      const lastViewedCourse = await Course.findById({
        _id: req.params.courseId,
      });

      const student = await Student.findById({ _id: req.user.id });

      const indexOfCourse = student.lastViewedCourse.indexOf(
        lastViewedCourse._id
      );
      if (indexOfCourse >= 0) {
      ...
      res
        .status(200)
        .send({
          message: "Updated the recently viewed course",
          lastViewedCourse,
        });
    } catch (error) {
      ...
      res
        .status(500)
        .send({
          message: "Couldn't updated the last viwed course",
          error: error.message,
        });
    }
  },
